...
def seleccionar_direcciones(calle, numero, df_direcciones):
    ...
    for i in range(df_direcciones.shape[0]):
        if df_direcciones['Nombre de la vía'][i].strip() == calle and df_direcciones['Número'][i] == int(numero): 
            seleccion1 = i + 1
    ...
    coordenadas = (int(df_direcciones['Coordenada X (Guia Urbana) cm'][seleccion1]), int(df_direcciones['Coordenada Y (Guia Urbana) cm'][seleccion1]))
...
def seleccionar_opcion_ruta(grafo_1, grafo_2):
    #Imprimimos las opcioens de ruta
    ...
    try:
        ...
        if seleccion == 1:
            grafo = grafo_1
            return "corta", grafo       #Devolvemos la opcion de ruta y el grafo que se va a utilizar con el 
        elif seleccion == 2:
            grafo = grafo_2
            return "rapida" , grafo
        else:
...
def encontrar_cruce_mas_cercano(direccion, cord_x, cord_y, lista_cruces):
    ...
    for _,cruce in cruces_df.iterrows():
        ...
        coord_x_cruce = pd.to_numeric(cruce['Coordenada X (Guia Urbana) cm (cruce)'], errors='coerce')
        coord_y_cruce = pd.to_numeric(cruce['Coordenada Y (Guia Urbana) cm (cruce)'], errors='coerce')
...
def gps_ruta(origen, destino, grafo_1, grafo_2):
    ...
    ruta = encontrar_ruta(cordenadas_cruce_origen, cordenadas_cruce_destino, tipo_ruta, grafo)
...
def instrucciones(instrucciones, giros, df_cruces):
    ...
    print(f'Usted comienza {instrucciones[0][1]} metros por {obtener_nombre(instrucciones[0][0], df_cruces)}')
    ...
    print(f'Continue por la calle  {obtener_nombre(instrucciones[final][0], df_cruces)} durante {instrucciones[final][1]}')
...
if __name__ == '__main__': 
    ...
    lista_cruces = cruces(df_cruces)
    grafo_1, grafo_2 = arrancar_gps(df_direcciones, df_cruces, lista_calles, lista_cruces)  #tarda sobre 2mins

    while True:
        print('DATOS DE ORIGEN')
        origen_calle = input('Calle:')
        origen_numero = input('Número:')
        coordenadas_orig, codigo_orig, direccion_orig = seleccionar_direcciones(origen_calle, origen_numero, df_direcciones)
        ...
        coordenadas_dest, codigo_dest, direccion_dest = seleccionar_direcciones(destino_calle, destino_numero, df_direcciones)
        print(direccion_dest)

        camino_minimo, grafo = gps_ruta(direccion_orig, direccion_dest, grafo_1, grafo_2)   #tarda sobre 2mins

        dicc_recorrido_calles, distancias = recorrido_por_calles(grafo,camino_minimo, coordenadas_orig, coordenadas_dest, codigo_orig, codigo_dest)
        instrucciones_, giros = generar_instrucciones(dicc_recorrido_calles, distancias, df_cruces)
